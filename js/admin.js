jQuery(document).ready( function(){
    ...
    function bindGalleryClick(){
        ...
        jQuery('.clear_image_button').on('click', function( event ){
            var this_p = jQuery(this).closest('p');
            this_p.next('.img-prev').html('');
            this_p.find('.image-url').val('').trigger('change');
        });
        ...
        jQuery('.upload_image_button').on('click', function( event ){
            ...
            wcp_image_widget.on( 'select', function() {
                ...
                this_p.find('.image-url').val(attachment.url).trigger('change');
                this_p.find('.image-title').val(attachment.title);
                this_p.find('.alttext').val(attachment.alt);
                this_p.next('.img-prev').html('<img src="'+attachment.url+'" width="100%" style="max-width: ' + attachment.width + 'px">');
            });
